    ...
    if (headerTitleRef.current && headerTextRef.current) {
      document.fonts.ready.then(() => {
        ...
        const tl = gsap.timeline({
          scrollTrigger: {
            trigger: headerTextRef.current, start: 'top 80%',
          },
        });
        ...
        tl.fromTo(afterRule, {cssRule: {opacity: 0, filter: 'blur(10px)'}}, {
    ...
    if (contentSubtitleRef.current && contentTextRef.current) {
      document.fonts.ready.then(() => {
        ...
        const contentTl = gsap.timeline({
          scrollTrigger: {
            trigger: contentRef.current, start: 'top 20%',
          },
        });

        contentTl.fromTo(contentSubtitleRef.current, {
          opacity: 0, filter: 'blur(10px)',
        }, {
          opacity: 1, filter: 'blur(0px)', duration: 1, ease: 'power3.out',
        });

        contentTl.fromTo(textLines, {
          opacity: 0, filter: 'blur(10px)', y: 30,
        }, {
          opacity: 1,
          filter: 'blur(0px)',
  ...
  useGSAP(() => {
    document.fonts.ready.then(() => {
      if (bottomTextRef.current && bottomContainerRef.current) {
        ...
        bottomTl.fromTo(bottomTextLines, {opacity: 0, y: 50}, {
          opacity: 1,
          y: 0,
          ease: 'power3.out',
          duration: 5,
        ...
        >
          <div className="content-bottom__image">
            <img
              ...
              alt="Luxury travel illustration showing a premium vacation experience"
              width={276}
              height={640}
            />
          </div>
          <div className="content-bottom-right">
            ...
            <a
              ...
              href="/"
              ref={bottomLinkRef}
              aria-label="Help me plan a luxury trip"
            >
              help me plan a trip
            </a>
          </div>
